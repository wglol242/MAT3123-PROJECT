...
이 코드는 기존의 2D U-Net 구조를 3D U-Net으로 변환한 것입니다.
3D U-Net은 의료 이미지 세그멘테이션(예: 3D MRI 또는 CT 데이터)과 같은 볼륨 데이터 처리에 사용됩니다.
"""
...
# 모델 테스트
model = simple_unet_model(128, 128, 128, 3, 4)  # (높이, 너비, 깊이, 채널, 클래스 수)
print(model.input_shape)  # 입력 크기 출력
print(model.output_shape)  # 출력 크기 출력
